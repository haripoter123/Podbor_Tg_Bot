...
async def generate_keys(context):
    ...
    while generating:
        for _ in range(BATCH_SIZE):
            ...
            # Не проверяем существующие ключи для тестирования
            # if private_key_hex in existing_keys:
            #     continue
            ...
            # Запись накопленных ключей в файл
            if len(generated_keys) >= BATCH_SIZE:
                write_batch_to_file()

            # Отправка сообщений пользователю каждые BATCH_SIZE попыток
            ...
            # Вывод информации в консоль каждые 100000 попыток
...
# Команда /stop для мгновенной остановки генерации
